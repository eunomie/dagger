...
func setSDKSkipRuntimeCodegen(configPath string) error {
	...
	// The engine's exporter writes a .gitignore next to dagger.json when
	// automaticGitignore is true (the default). We just flipped that to
	// false, so remove the stale file — otherwise Host.directory(gitignore:true)
	// would keep excluding the very files we need committed.

...
{{ "" }}    def {{ .Name | FormatMethod }}
	...
	{{- else }}
      ...
      unless opts.nil?
        ...
        _node_args['{{ .Name }}'] = opts.{{ .Name | FormatArg }} unless opts.{{ .Name | FormatArg }}.nil?
		{{- end }}
      end
		{{- end }}
		{{- if Solve . }}
      n = {{$outputType}}.new(self, @client, '{{.Name}}', _node_args)
      @client.invoke(n)
		{{- else }}
      {{$outputType}}.new(self, @client, '{{.Name}}', _node_args)
		{{- end }}
	{{- end }}
    end
{{- end }}

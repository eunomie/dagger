...
`)
	...
	require.NoError(t, err)
	addedCfg, err := workspacecfg.ParseConfig([]byte(config))
	require.NoError(t, err)
	require.Equal(t, []string{"target-alias"}, addedCfg.SDKs["go"].Scopes["."].Clients)
...
`)
	...
	requireScopeClients := func(ctx context.Context, t *testctx.T, ctr *dagger.Container, scope string, clients []string) {
		...
		require.NoError(t, err)
		require.Equal(t, clients, cfg.SDKs["go"].Scopes[scope].Clients)
	}

	t.Run("workspace root scope", func(ctx context.Context, t *testctx.T) {
		added := base.
			With(daggerNonNestedExec("--auto-apply", "module", "init", "go", "--name", "app", "--path", ".")).
			With(daggerNonNestedExec("--auto-apply", "module", "client", "add", "go", "./dep"))

		out, err := added.CombinedOutput(ctx)
		require.NoError(t, err, out)
		require.NotContains(t, out, "no changes to apply")

		requireScopeClients(ctx, t, added, ".", []string{"dep"})
		client, err := added.File("/work/internal/dagger/clients/dep.gen.go").Contents(ctx)
		require.NoError(t, err)
		...
		out, err = regenerated.CombinedOutput(ctx)
		require.NoError(t, err, out)
		client, err = regenerated.File("/work/internal/dagger/clients/dep.gen.go").Contents(ctx)
		require.NoError(t, err)
		require.Contains(t, client, "Code generated by module-max-lifecycle")
	})

	t.Run("subdirectory scope", func(ctx context.Context, t *testctx.T) {
		added := base.
			With(daggerNonNestedExec("--auto-apply", "module", "init", "go", "--name", "app2")).
			WithWorkdir("/work/.dagger/modules/app2").
			With(daggerNonNestedExec("--auto-apply", "module", "client", "add", "go", "../../../dep"))
		...
		requireScopeClients(ctx, t, added, ".dagger/modules/app2", []string{"dep"})
		client, err := added.File("/work/.dagger/modules/app2/internal/dagger/clients/dep.gen.go").Contents(ctx)
		require.NoError(t, err)
		require.Contains(t, client, "Code generated by module-max-lifecycle")
	})

	t.Run("installed module alias", func(ctx context.Context, t *testctx.T) {
		added := base.
			With(daggerNonNestedExec("--auto-apply", "module", "init", "go", "--name", "app3", "--path", ".")).
			With(daggerNonNestedExec("module", "install", "./dep", "--name", "dep-alias")).
			With(daggerNonNestedExec("--auto-apply", "module", "client", "add", "go", "dep-alias"))

		out, err := added.CombinedOutput(ctx)
		require.NoError(t, err, out)
		require.NotContains(t, out, "no changes to apply")

		requireScopeClients(ctx, t, added, ".", []string{"dep-alias"})
		client, err := added.File("/work/internal/dagger/clients/dep.gen.go").Contents(ctx)
		require.NoError(t, err)
		require.Contains(t, client, "Code generated by module-max-lifecycle")
	})

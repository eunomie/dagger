...
//
// The entrypoint is code the module's author chose, so it never receives the
// caller's workspace: that holds every file of whoever loaded the module, and
// the caller passed none of them. A module that needs the caller's workspace
// declares a Workspace argument on a function, which the caller supplies.
//
// The root is the module's context, never the tree the caller runs in. A git
// source gets its repository at the pinned commit and a directory source the
// directory it was created from; both are the module's own. A local source, or
// any source loaded from a workspace, gets the context the engine loaded for
// it: the files its manifest includes. Its host tree can be the caller's
// workspace, so nothing outside what the module declared is reachable.
func Workspace(
	ctx context.Context,
	dag *dagql.Server,
	src dagql.ObjectResult[*core.ModuleSource],

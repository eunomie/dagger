...
// working directory is the module's. Nothing else of the caller's workspace is
// in it.
//
// The caller's config decides what is in scope. The engine reads it here, from
// the caller's own workspace, so the module cannot widen it. Each client's files
// are the engine's own include-filtered load of that client, not its host tree.
//

...
// keyed by its canonical repo basename ("go-sdk", "php-sdk"), matching
// `dagger sdk install`, so the SDK install cannot collide with an unrelated
// module legitimately named "go". External refs and custom/local SDK names
// keep their existing basename.

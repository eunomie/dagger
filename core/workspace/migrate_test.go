...
// the canonical repo basename (go -> go-sdk) so it cannot collide with an
// unrelated module named "go", with the migrated module recorded under
// [[modules.<name>.as-sdk.modules]].
func TestPlanMigrationWritesAsSDK(t *testing.T) {
  ...
  "toolchains": [
	...
	require.Contains(t, configData, "[[modules.go-sdk.as-sdk.modules]]")
...
func TestMigrationSDKInstallName(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"go", "go-sdk"},         // builtin short name -> repo basename
		{"php", "php-sdk"},       // builtin short name -> repo basename
		{"php@v0.18", "php-sdk"}, // versioned builtin
...
// basename; it is recorded under a distinct name instead.
func TestPlanMigrationKeepsUnrelatedModuleNamedLikeSDK(t *testing.T) {
  ...
  "source": "src",
  "toolchains": [
    {"name": "go-sdk", "source": "github.com/acme/go-sdk"}
  ]
}`)
	...
	unrelated, ok := cfg.Modules["go-sdk"]
	require.True(t, ok)
...
// where an unrelated module shares both the SDK's basename and its bare source
// string ("go"): the string means a runtime in SDK context but a local path in
// module context, so it must not be treated as the same install.

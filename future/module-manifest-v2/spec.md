...
directory at the module directory. It is never the caller's workspace: the
entrypoint is code the module's author chose, and the caller passed it no
files. A module function that declares a `Workspace` argument receives the
caller's workspace through that argument, which the caller supplies.
...
engine reads the caller's config itself, so the module cannot widen the scope.
The entrypoint resolves those clients through `Workspace.moduleSource` as it
would in the caller's workspace.
